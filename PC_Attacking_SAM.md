...
All we must do to create the share is run smbserver.py -smb2support using python, give the share a name (`CompData`) and specify the directory on our attack host where the share will be storing the hive copies (`/home/ltnbob/Documents`). Know that the `smb2support` option will ensure that newer versions of SMB are supported. If we do not use this flag, there will be errors when connecting from the Windows target to the share hosted on our attack host.	
```shell-session
...
Assessment:
The reason this module was doable in the first place is that Bob has admin account. Otherwise step 2 invoving reg.exe would have failed
1. Login to the RDP
2. copy the SAM file to another location using reg.exe
3. Run an smb on the attack host
4. move the cloned sam file from the windows to the attack host
5. extract the hash to a file
6. offline cracking the password
7. Use crackmapexec to dump lsa

alternative is that we can already dump hashes with crackmapexec